 ...
 */
class VerifyEmail extends CustomerAccount implements NotificationFactoryInterface
{
    /**
     * {@inheritdoc}
     */
    public function generate(Model $submission, $actionKey, Model $template = null, array $args)
    {
        ...
        if (null === $template) {
            $args['subject'] = sprintf('Verify your email for %s', $app->get('name'));
        }
    ...
    {
        return 'customer-account' === $submission->get('sourceKey') && 'verify-email' === $actionKey;
    }
}
